...
export default function StarMessages(props) {
    ...
    async function starAndUnstarMessage() {
      ...
      const response = await fetch(`http://localhost:5000/starAndUnstar/messages?selectedMessages=${JSON.stringify(selectedMessagesList)}&starStatus=${props.star > 0}&ID=${props.room}`, {
        method: 'PUT',
        credentials: "include",
        headers: {
  ...
  return (
    <div>
      {
        props.star ? <span onClick={ () => starAndUnstarMessage() } className='mx-1 px-2 text-3xl rounded-full hover:bg-violet-400'>&#9733;</span> : 
