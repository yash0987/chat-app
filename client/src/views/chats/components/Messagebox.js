...
export default function Messagebox(props) {
  ...
  return (
    <section onMouseDown={ () => triggerHoldEvent() } onMouseUp={ () => stopHoldEvent() } onMouseLeave={ () => stopHoldEvent() } id={props.element.messageID} className = {`${ highlightSelectMessage && toggleFeaturesState ? `${theme.bg500} bg-opacity-20` : 'bg-none' } select-none rounded-md grid grid-flow-col ${props.element.senderID !== user.googleID ? 'justify-start' : 'justify-end'} my-1 p-1`}>
      ...
      <div className = {`group relative whitespace-pre-wrap break-words leading-5 max-w-lg mx-2 px-2 py-1 rounded-md ${props.element.senderID !== user.googleID ? `${theme.bg500} text-white` : 'bg-gray-200 text-gray-600' }`}>
        { props.element.senderID !== user.googleID ? <p className={`text-xs ${theme.text200}`}>@{ props.element.senderName }</p> : null }
        { downloadButton }
        { replyToMessage }
        { message }
